...
class Cilo {
  ...
  async createTenant(subdomain) {
    ...
    const migrationUmzug = this._initializeUmzug(this.tenantConnections[subdomain], tenantGlob)
  ...
  async buildTenantConnections() {
    ...
    for (const tenant of tenants) {
      const temp = {}
      if(tenant.host) {
        temp.host = tenant.host;
      }
      this.tenantConnections[tenant.subdomain] = new this.Sequelize(tenant.dbName, tenant.username, tenant.password, {...this.config, ...temp})
    }
  }
  ...
  async updateTenant() {
    const tenants = await this.getDbTenants()
    for (const tenant of tenants) {
      if (!this.tenantConnections[tenant.subdomain]) {
        this.tenantConnections[tenant.subdomain] = new this.Sequelize(tenant.dbName, tenant.username, tenant.password, this.config)
        this.tenantORMs[tenant.subdomain] = this._importModels(this.tenantConnections[tenant.subdomain])
      }
    }
  }

  async setCurrentORM(subdomain) {
    if (this.tenantORMs[subdomain] !== false) {
      if (this.tenantORMs[subdomain] === undefined) {
        await this.updateTenant();
        if (!this.tenantORMs[subdomain]) {
          this.tenantORMs[subdomain] = false;
          console.log(`No tenant with subdomain ${subdomain}`)
          throw new Error(`premission denined`);
        }
      }
    } else if(this.tenantORMs[subdomain] === false) {
      console.log(`No tenant with subdomain ${subdomain}`)
      throw new Error(`premission denined`);
    }
  ...
  _importModels(connection) {
    ...
    Object.keys(modelFiles).map(k => {
      modelFiles[k].forEach(file => {
        const model = require(file)(
          connection,
          this.Sequelize.DataTypes
        );
        if (!orm[k]) orm[k] = {}
        orm[k][model.name] = model
        // if (orm[k][model.name].associate) {
        //   orm[k][model.name].associate(orm);
        // }
      })

    })

    Object.keys(orm).forEach((modelParentName) => {
      Object.keys(orm[modelParentName]).forEach((modelName) => {
        if (orm[modelParentName][modelName].associate) {
          orm[modelParentName][modelName].associate(orm);
        }
      })
    })
...
const _getModelFiles = inputpath => {
  const out = {}
  fs
    ...
    .forEach(subDir => {
      fs
        .readdirSync(inputpath + '/' + subDir.name)
        .filter(file => {
          return (file.indexOf('.') !== 0) && file !== 'index.js' && (file.slice(-3) === '.js');
        })
        .forEach(file => {
          // const model = require(path.join(__dirname, subDir.name, file))(sequelize, Sequelize.DataTypes);
          // if (!db[subDir.name]) db[subDir.name] = {};
          // db[subDir.name][model.name] = model;
          // if (db[subDir.name][model.name].associate) {
          //   db[subDir.name][model.name].associate(db);
          // }
          if (!out[subDir.name]) out[subDir.name] = []
          out[subDir.name].push(path.join(inputpath, subDir.name, file))
        });
    });
  console.log(out)
  return out;
