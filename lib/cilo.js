...
class Cilo {
  ...
  async createTenant(subdomain) {
    ...
    const tenantGlob = this._buildMigrationGlob().tenantDb
  ...
  async migrateMainDb() {
    const mainDbGlob = this._buildMigrationGlob().mainDb
  ...
  setCurrentORM(subdomain) {
    if (! this.tenantORMs[subdomain]) {
      throw new Error(`No tenant with that subdomain ${subdomain}`)
    }
  ...
  _buildMigrationGlob() {
    const mainDbModels = this.config.tenantify.models.main_db
    const tenantDbExcludedModels = this.config.tenantify.models.tenant_db_excluded
    // append asterisk to both sides of each model name and join them with a pipe
    const mainDbModelsRegex = mainDbModels.map(model => `*${model}*.js`).join(',')
    // const tenantDbExcludedModelsRegex = tenantDbExcludedModels.map(model => `*${model}*.js`).join('|')
    const mainDbGlob = path.join(this.modelsPath, `../migrations/{${mainDbModelsRegex}}`)
    // tenant glob with all migrations
    const tenantDbGlob = path.join(this.modelsPath, `../migrations/*.js`)
    ...
    const migrationFilesArray = {
      mainDb: glob.sync(options.mainDbGlob),
      tenantDb: glob.sync(options.tenantDbGlob)
    }
    // explode the array into a string with comma separated values, and put inside a curly braces
    const migrationFiles = {
      mainDb: `{${migrationFilesArray.mainDb.join(',')}}`,
      tenantDb: `{${migrationFilesArray.tenantDb.join(',')}}`
    }
